...
static func override_body_pattern(species_id: String, mesh: MeshInstance, pattern: Resource):
	var Lure = mesh.get_node_or_null("/root/Lure") # this is silly but it works!!! :3
	
	if (not pattern is LureCosmetic and not pattern is CosmeticResource) or not Lure:
		return
...
static func get_bark_id(player: Actor, equipped_species: String) -> Array:
	equipped_species = equipped_species.validate_node_name()

	if equipped_species == "species_cat" or equipped_species == "species_dog":
		equipped_species = equipped_species.trim_prefix("species_")
